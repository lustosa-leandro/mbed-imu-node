...
unsigned char *mBinPutLong(unsigned char *p, long d) {
...
// pointer to point to the next data item.
long mBinGetLong(unsigned char **p) {
    long d;
    unsigned char *q = *p;
    d = *q++;
    d = (d<<8)|(*q++);
...
// returned.
unsigned char *mBinGetPacket(mBinPacketizer p, unsigned char *ck0, unsigned char *ck1) {
    int ch;
    unsigned char uc;
